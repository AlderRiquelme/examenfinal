...
public final class Dashboard extends javax.swing.JFrame {
    ...
    if (total != 0) {
        if (!btnTotal.isEnabled()) {
            try {
                // Imprimir la factura después del pago
                jTextArea.print();
            } catch (PrinterException ex) {
                Logger.getLogger(Dashboard.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            // Mensaje si no se ha calculado el precio total
            JOptionPane.showMessageDialog(null, "Por favor, calcula el precio total antes de confirmar la compra.");
        }
    } else {
        // Mensaje si no se ha comprado ningún producto
        JOptionPane.showMessageDialog(null, "No has comprado ningún producto. La factura no se puede generar.");
    }
